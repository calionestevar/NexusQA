#!/bin/bash
#
# Generate-ObserverNetworkTemplate.sh
...
PROJECT_ROOT="${1:-.}"
CPP_SOURCE_PATH="$PROJECT_ROOT/Source/FringeNetwork/Private/ObserverNetworkTemplate.cpp"
HTML_OUTPUT_PATH="$PROJECT_ROOT/Source/FringeNetwork/Private/ObserverNetworkDashboard.html"
...
if sed -n '/R"(/,/)"/{s/^.*R"(//;s/)".*$//p;}' "$CPP_SOURCE_PATH" > "$HTML_OUTPUT_PATH"; then
    ...
    echo ""
    echo "Generated from: ObserverNetworkTemplate.cpp"
